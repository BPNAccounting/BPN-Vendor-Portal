...
function formatTin(value: string, type: 'SSN' | 'EIN'): string {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <FormField label="DBA (Doing Business As)" hint="If different from legal name" error={errors.dba}>
          <Input
            value={data.dba}
      ...
      {/* TIN */}
      <div>
        <h3 className="text-sm font-semibold text-slate-700 mb-3">Taxpayer Identification Number (TIN)</h3>
        ...
        <FormField
          label={data.tin_type === 'SSN' ? 'Social Security Number' : 'Employer Identification Number'}
          required
          error={errors.tin}
          hint={data.tin_type === 'SSN' ? 'Format: XXX-XX-XXXX' : 'Format: XX-XXXXXXX'}
        >
          <MaskedInput
            value={data.tin}
            onChange={handleTinInput}
            placeholder={data.tin_type === 'SSN' ? '000-00-0000' : '00-0000000'}
            error={errors.tin}
            maxLength={data.tin_type === 'SSN' ? 11 : 10}
            inputMode="numeric"
          />
        </FormField>
      </div>
