...
export default function StepFour({ data, onChange, errors, onEdit }: Props) {
  ...
  const stateRegions = isCA ? CA_PROVINCES : US_STATES;
  const regionLabel = (code: string) => stateRegions.find(s => s.value === code)?.label ?? code;
  ...
  const maskedTin = data.tin
    ? data.tin_type === 'SSN'
      ...
      : data.tin_type === 'EIN'
        ...
        : `•••••${data.tin.replace(/\D/g, '').slice(-4)}` // BN
    : '';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        <span className="text-lg">{isCA ? '🇨🇦' : '🇺🇸'}</span>
        <span className="text-sm font-medium text-slate-600">{isCA ? 'Canadian Vendor' : 'US Vendor'}</span>
        <button type="button" onClick={() => onEdit(1)} className="text-xs text-bpn-600 hover:text-bpn-700 font-medium ml-1">
          Change
        </button>
      </div>
      ...
      <div className="space-y-4">
        <ReviewSection title="Step 1 — Company & Tax Info" step={1} onEdit={onEdit}>
          <div className="space-y-1.5">
            ...
            {!isCA && <Row label="Tax Classification" value={taxLabel + (data.tax_classification === 'llc' ? ` (${data.tax_classification_llc})` : '') + (data.tax_classification === 'other' ? `: ${data.tax_classification_other}` : '')} />}
            {!isCA && <Row label="Exempt Payee Code" value={data.exempt_payee_code} />}
            {!isCA && <Row label="FATCA Exemption Code" value={data.exempt_fatca_code} />}
            ...
            <Row label={data.tin_type === 'BN' ? 'Business Number' : data.tin_type} value={maskedTin} />
          </div>
        </ReviewSection>
        ...
        <ReviewSection title="Step 3 — Payment Authorization" step={3} onEdit={onEdit}>
          <div className="space-y-1.5">
            ...
            {isCA ? (
              <>
                <Row label="Transit Number" value={data.ca_transit_number} />
                <Row label="Institution Number" value={data.ca_institution_number} />
              </>
            ) : (
      ...
      <div className="space-y-3">
        ...
        {/* ACH / EFT authorization — all vendors */}
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div>
              <p className="text-sm font-medium text-slate-800 mb-1">
                {isCA ? 'EFT Payment Authorization' : 'ACH Payment Authorization'}
              </p>
              <p className="text-xs text-slate-600 leading-relaxed">
                I authorize the {isCA ? 'EFT' : 'ACH'} payment terms described in Step 3 and authorize Bare Performance Nutrition to initiate electronic fund transfers to the designated account.
              </p>
            </div>
          </label>
      ...
      <div className="flex items-start gap-2 text-xs text-slate-500">
        ...
        <p>
          Your information is transmitted securely. Sensitive data including your {isCA ? 'Business Number' : 'TIN'} and bank account information is encrypted before storage and is accessible only to authorized BPN personnel.
        </p>
      </div>
    </div>
  );
